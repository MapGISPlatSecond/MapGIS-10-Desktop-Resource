...
Zondy.Map.MultiTileLayer = OpenLayers.Class(OpenLayers.Layer.Grid, Zondy.Service.HttpRequest, {
     ...
     *
     * @param name
     * @param hdfNames  [{name:"",minlev:0,maxlev:5},{name:"",minlev:5,maxlev:6}]
     * @param options
     */
    initialize: function (name, hdfNames, options) {

        var newArguments = [];

        this.hdfNames = hdfNames;
    ...
    clone: function (obj) {
        ...
        if (obj == null) {
            obj = new Zondy.Map.MultiTileLayer(this.name,
                this.hdfNames,
                this.getOptions());
        }
    ...
    getURL: function (bounds) {
        ...
        var hdfName;
        for(var i=0 ; i < this.hdfNames.length;i++){
            if( zoom >= this.hdfNames[i].minLev && zoom <=this.hdfNames[i].maxLev){
                hdfName = this.hdfNames[i].name;
            }
        }

        var path = "/" + hdfName + "/" + z + "/" + y + "/" + x;
    ...
    CLASS_NAME: "OpenLayers.Layer.MultiTileLayer"
});
