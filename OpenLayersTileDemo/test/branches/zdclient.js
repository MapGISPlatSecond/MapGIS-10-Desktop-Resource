...
Zondy.Map.Doc = OpenLayers.Class(OpenLayers.Layer.Grid, Zondy.Service.HttpRequest, {
    ...
    getURL: function (bounds) {
        ...
        if (this.cache) {
            ...
            newParams.projection = $.toJSON(this.proj);
...
Zondy.Map.TianDiTu = OpenLayers.Class(OpenLayers.Layer.Grid, Zondy.Service.HttpRequest, {
...
Zondy.Object.WebGraphicsInfo = OpenLayers.Class({
    ...
    /// <summary>Zondy.Enum.FeatureType枚举类型，取值范围： 1（PntInfo）,2（LinInfo）,3（RegInfo）</summary>
    InfoType: null,

    /// <summary>线信息对象,CLineInfo</summary>
    LinInfo: null,

    /// <summary>点信息对象,CPointInfo</summary>
    PntInfo: null,

    /// <summary>区信息对象,CRegionInfo</summary>
    RegInfo: null,
...
Zondy.Object.Rectangle = OpenLayers.Class(Zondy.Object.Tangram, {
    ...
    setByOL: function (openlayersRect) {
        ...
        this.xmin = openlayersRectangle.components[0].components[3].x;
        this.ymin = openlayersRectangle.components[0].components[3].y;
        this.xmax = openlayersRectangle.components[0].components[1].x;
        this.ymax = openlayersRectangle.components[0].components[1].y;
    },
...
Zondy.Object.FeatureSet = OpenLayers.Class({
    ...
    * 属性结构
    * {Array}
    * {Zondy.Object.CAttStruct}in Array
    */
    AttStruct: null,
    ...
    */
    ...
    addFeature: function (features) {
        ...
        /// <param name="name" type="{Array} | {Zondy.Object.Feature} | {Object}">一组要素，或者一个要素</param>
...
Zondy.Object.PolyLine = OpenLayers.Class(Zondy.Object.Tangram, {
    ...
    setByOL: function (openlayersLine) {
        ...
        var i;
        for (i = 0; i < len; i++) {
            this.pointArr[i] = new Zondy.Object.Point2D(openlayersLine.components[0].components[i].x, openlayersLine.components[0].components[i].y);
        }
    },
...
Zondy.Service.AnalysisBase = OpenLayers.Class(Zondy.Service.HttpRequest, {
    ...
    execute: function (onSuccess, way, isAsy, f) {
        ...
        this.ajax(null, data, onSuccess, way);
    }
})
/*----------------------------------------------------------------------------------------------------------------------------*/
...
Zondy.Service.TopAnalysis = OpenLayers.Class(Zondy.Service.GeometryAnalysisBase, {
    /// <summary>叠加分析类,您只应该对pnt,line,reg3个属性中的一个赋值</summary>
...
Zondy.Service.Catalog.MapDoc = OpenLayers.Class(Zondy.Service.Catalog.CatalogServiceBase, {
    ...
    addLayer: function (addLayerInfos, onSuccess) {
        ...
        /// <param name="addLayerInfo" type="Array,Zondy.Service.Catalog.AddMapLayerInfo in an Array">需要添加的图层</param>
        /// <param name="onSuccess" type="Function">回调函数</param>
    ...
    creatDoc: function (addDocInfo, onSuccess) {
        /// <summary>创建文档（POST）</summary>
        /// <param name="addDocInfo" type="Zondy.Service.Catalog.AddDocInfo">需要创建的文档信息</param>
        /// <param name="onSuccess" type="Function">回调函数</param>
...
Zondy.Service.EditServiceBase = OpenLayers.Class(Zondy.Service.HttpRequest, Zondy.Service.DocLayer, {
...
Zondy.Service.EditDocFeature = OpenLayers.Class(Zondy.Service.EditServiceBase, Zondy.Service.DocLayer, {
    ...
    add: function (features, onSuccess) {
        ...
        var key = this.getFullUrl();
        this.p_onOperSuccess = onSuccess;
        this.featureSet = features;
        this.checkAttValues(key, features, this.onRepairAttValues);
    },
    ...
    p_onOperSuccess: null,
    onRepairAttValues: function (success) {
        if (success) {
            var url = this.getFullUrl() + "?f=" + this.resultFormat;
            this.ajax(url, this.featureSet, this.p_onOperSuccess, "POST");
        }
    },

    checkAttValues: function (key, featureSet, onSuccess) {
        /// <summary>将values填充为REST服务需要的形式</summary>

        this.p_curCahceKey = key;
        if (window.Zondy.ATT_STURCT_CACHE.containsKey(key)) {
            var obj = window.Zondy.Att_STURCT_CACHE.get(key);
            this.p_containsKey = true;
            this.p_GetAttNamesOnSuccess(obj);
        }
        else {
            this.p_containsKey = false;
            this.p_featureSet = featureSet;
            this.p_onCheckAttValuesSuccess = onSuccess;
            var url = this.getBaseUrl() + "docs/sample_wuhan/0/1/query" + "?f=json&objectIds=23";
            this.ajax(url, null, this.p_GetAttNamesOnSuccess, "GET");
        }
    },

    /***********************************************Private**************************************/

    //本类内部使用，用于存贮AttValues
    p_featureSet: null,

    p_onCheckAttValuesSuccess: null,

    // 全局AttSTruct缓存中是否已经存在当前key
    p_containsKey: false,

    // 当前key
    p_curCahceKey: null,

    p_GetAttNamesOnSuccess: function (obj) {
        /// <summary>补全用户的attvalues数组</summary>
        var attNames = obj.AttStruct.FldName;
        if (!this.p_containsKey) {
            window.Zondy.ATT_STURCT_CACHE.add(this.p_curCahceKey, obj);
            this.p_containsKey = true;
        }
        var featureArray = this.p_featureSet.SFEleArray;
        for (var i = 0; i < featureArray.length; i++) {
            var attValues = featureArray[i].AttValue
            if (!(attValues instanceof Array)) {
                featureArray[i].AttValue = this.getFullAttValues(attNames, attValues);
            }
        }
        this.p_onCheckAttValuesSuccess(true);
    },

    getFullAttValues: function (attNames, attValues) {
        var newAttValues = new Array();
        for (var poperty in attNames) {
            var attName = attNames[poperty];
            if (attValues.hasOwnPoperty(attName)) {
                //如果用户已经指定了某属性的值，则输入
                newAttValues.push(attValues[attName]);
            }
            else {
                // 如果用户没有指定，则补空字符串
                newAttValues.push("");
...
Zondy.Service.QueryServiceBase = OpenLayers.Class(Zondy.Service.HttpRequest, Zondy.Service.DocLayer, {
...
Zondy.Service.QueryDocFeature = OpenLayers.Class(Zondy.Service.QueryServiceBase, {
    ...
    initialize: function (queryParam, docName, layerIndex, options) {
        /// <summary>构造函数</summary>
        /// <param name="queryParam" type="Zondy.Service.QueryByGeometryParameter">几何查询的参数类</param>
        /// <param name="docName" type="String">文档名称</param>
        /// <param name="layerIndex" type="Interger">图层序号</param>
        /// <param name="options" type="Object">属性赋值对象</param>
        $.extend(this, options);
...
Zondy.Service.QueryParameter = OpenLayers.Class(Zondy.Service.QueryParameterBase, {
    ...
    getParameterURL: function () {
        ...
        paramUrl += "&geometryType=" + this.geometry.geometryType;
    ...
    getParameterObject: function () {
        /// <summary>获取相关参数的Object形式</summary>

        if (this.objectIds != null) {
            var obj = { f: this.resultFormat, objectIds: this.objectIds }
            return obj;
        };

        var obj = {
            page: this.pageIndex.toString(), pageCount: this.recordNumber.toString(),
            f: this.resultFormat
        };

        if (this.geometry != null) {
            obj.geometry = this.geometry.toString();
            if (this.geometry instanceof Zondy.Object.Point2D) {
                obj.geometryType = "point";
            }
            if (this.geometry instanceof Zondy.Object.Polygon) {
                obj.geometryType = "polygon";
            }
            if (this.geometry instanceof Zondy.Object.PolyLine) {
                obj.geometryType = "line";
            }
            if (this.geometry instanceof Zondy.Object.Rectangle) {
                obj.geometryType = "rect";
            }
            if (this.geometry instanceof Zondy.Object.Circle) {
                obj.geometryType = "circle";
            }
            //obj.geometryType = this.geometry.geometryType;
        }
...
Zondy.Service.Catalog.AddMapLayerInfo = OpenLayers.Class({
...
Zondy.Service.Catalog.AddMapGroupInfo = OpenLayers.Class({
...
Zondy.Service.Catalog.AddMapInfo = OpenLayers.Class({
    ...
    /// <summary>Array,Zondy.Service.Catalog.AddMapLayerInfo in an Array，地图下的图层信息</summary>
    LayerInfos: null,

    /// <summary>Array,Zondy.Service.Catalog.AddMapGroupInfo in an Array，地图下的组信息</summary>
    GroupInfos: null,
...
Zondy.Service.Catalog.AddDocInfo = OpenLayers.Class({
